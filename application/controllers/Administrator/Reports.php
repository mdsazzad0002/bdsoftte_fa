...
class Reports extends CI_Controller {
   ...
   public function specialReport1()
    {
        $access = $this->mt->userAccess();
        ...
        $datas['title'] = "Cash Statement";
        $data['content'] = $this->load->view('Administrator/reports/special1', $datas, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function special_report_all_data()
    {
        ...
        $where = ["Status = 'a'"]; // Required for tbl_salesmaster

        $dateFrom = !empty($data->fromDate) ? $this->db->escape($data->fromDate) : null;
        $dateTo   = !empty($data->toDate) ? $this->db->escape($data->toDate) : null;
        $user     = !empty($data->userFullName) ? $this->db->escape($data->userFullName) : null;

        if ($dateFrom && $dateTo) {
            $where[] = "SaleMaster_SaleDate BETWEEN $dateFrom AND $dateTo";
        }
        ...
        $saleWhere = implode(" AND ", $where);

        // Subquery filters for other tables
        $saleReturnWhere = "1";
        $exchangeWhere = "1";

        if ($dateFrom && $dateTo) {
            $saleReturnWhere .= " AND SaleReturn_ReturnDate BETWEEN $dateFrom AND $dateTo";
            $exchangeWhere .= " AND `date` BETWEEN $dateFrom AND $dateTo"; // Replace with actual date column if different
        }

        if ($user) {
            $saleReturnWhere .= " AND AddBy = $user";
            $exchangeWhere .= " AND AddBy = $user";
        }

        $query = "
            SELECT 
                sale_totals.AddBy,
                sale_totals.totalsales,
                sale_totals.cashamount,
                sale_totals.changeamount,
                sale_totals.bankamount,
                IFNULL(sr_totals.returnamount, 0) AS returnamount,
                IFNULL(exchange_totals.exchangeamount, 0) AS exchangeamount
            FROM 
                (
                    SELECT 
                        AddBy, 
                      
                        SUM(SaleMaster_TotalSaleAmount) AS totalsales,
                        SUM(SaleMaster_cashPaid) AS cashamount,
                        SUM(returnAmount) AS changeamount,
                        SUM(SaleMaster_bankPaid) AS bankamount
                    FROM tbl_salesmaster 
                    WHERE $saleWhere AND SaleMaster_branchid = $brunch 
                    GROUP BY AddBy
                ) AS sale_totals

            LEFT JOIN (
                SELECT AddBy, SUM(SaleReturn_ReturnAmount) AS returnamount
                FROM tbl_salereturn
                WHERE $saleReturnWhere AND SaleReturn_brunchId = $brunch 
                GROUP BY AddBy
            ) AS sr_totals ON sr_totals.AddBy = sale_totals.AddBy

            LEFT JOIN (
                SELECT AddBy, SUM(total) AS exchangeamount
                FROM tbl_exchange
                WHERE $exchangeWhere AND branchId = $brunch 
                GROUP BY AddBy
            ) AS exchange_totals ON exchange_totals.AddBy = sale_totals.AddBy
        ";
